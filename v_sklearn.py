...
file_number = 22
blue_crystal = True
include_batchnorm_and_dropout = False
# Hyperparameters

#this is for set optimizer, set batch size and set learning rate

test_inputs = np.empty((0, 4))

for e_layers in range(3,0,-1):
    for d_layers in range(3,0,-1):
        for e_dim in range(10, 1110, 100):
            for d_dim in range(10, 1110, 100):
                test_inputs = np.append(test_inputs,[[e_dim,d_dim,e_layers,d_layers]],axis=0)

test_order = np.arange(np.shape(test_inputs)[0])
test_order = np.random.choice(test_order, size=np.shape(test_order)[0], replace=False)

save_results = np.empty((0,5))


def train_vae(intial_guess):
    t0 = time.time()
    ...
    save_interval = 2500
    xent_factor = 1000
    intermediate_dimE = intial_guess[0]
    intermediate_dimD = intial_guess[1]
    layersD = intial_guess[3]
    layersE = intial_guess[2]
    latent_dim = 4
    optimizer_choice = 0
    learning_rate = 0.001

    print(' ')
    print('Guess:',intial_guess)
    print(' ')
    ...
    if include_batchnorm_and_dropout == False:
        if layersD == 1:
            decoder = Sequential([
                Dense(int(intermediate_dimD), input_dim=latent_dim, activation='relu'),
                Dense(original_dim, activation='sigmoid')
            ])
        elif layersD == 2:
            decoder = Sequential([
                Dense(int(intermediate_dimD), input_dim=latent_dim, activation='relu'),
                Dense(int(intermediate_dimD), activation='relu'),
                Dense(int(original_dim), activation='sigmoid')
            ])
        elif layersD == 3:
            decoder = Sequential([
                Dense(int(intermediate_dimD), input_dim=latent_dim, activation='relu'),
        ...
        if layersE == 1:
            ...
            h = Dense(int(intermediate_dimE), activation='relu')(x)
        if layersE == 2:
            x = Input(shape=(int(original_dim),))
            h_2 = Dense(int(intermediate_dimE), activation='relu')(x)
            h = Dense(int(intermediate_dimE), activation='relu')(h_2)
        if layersE == 3:
            x = Input(shape=(int(original_dim),))
            h_2 = Dense(int(intermediate_dimE), activation='relu')(x)
            h_3 = Dense(int(intermediate_dimE), activation='relu')(h_2)
            h = Dense(int(intermediate_dimE), activation='relu')(h_3)
    else:
        if layersD == 1:
            decoder = Sequential([
    ...
    if blue_crystal == True:
        X_train = np.load('/mnt/storage/scratch/am13743/gan_training_data_weights/sample_neg13_0_0.npy')
        min_max = np.load('/mnt/storage/scratch/am13743/gan_training_data_weights/min_max_neg13_0_0.npy')
    else:
        # X_train = np.load('/Users/am13743/Desktop/GANs/thomas sample/sample_neg13_0_0.npy')
        # min_max = np.load('/Users/am13743/Desktop/GANs/thomas sample/min_max_neg13_0_0.npy')
        X_train = np.load('sample_neg13_0_0.npy')
        min_max = np.load('min_max_neg13_0_0.npy')
    ...
    for epoch in range(0, epochs):
        ...
        legit_images = X_train[random_indicies].reshape(int(batch), 4)
        ...
        if save == True:
            ...
            latent_space = encoder.predict(X_train ,batch_size=batch_size)

            # plt.hist([latent_space[:,0],latent_space[:,1],latent_space[:,2],latent_space[:,3]], bins=50, histtype='step')
...
# minimize(train_vae, intial_guess ,method='BFGS',bounds=((10,1000),(10,1000),(1,3),(1,3)))
# basinhopping(train_vae, intial_guess, niter=100)

for test_index in test_order:
    out = train_vae(test_inputs[test_index])
    save_results = np.append(save_results,[[out,test_inputs[test_index][0],test_inputs[test_index][1],test_inputs[test_index][2],test_inputs[test_index][3]]],axis=0)
    with open("test_output/save_results.txt", "a") as myfile:
        myfile.write('%.3f %.3f %.3f %.3f %.3f\n'%(out,test_inputs[test_index][0],test_inputs[test_index][1],test_inputs[test_index][2],test_inputs[test_index][3]))
